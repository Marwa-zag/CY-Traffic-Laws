...
const loadQuestions = async () => {
    try {
        const response = await fetch('exercice_sup1_questions.json');
        if (!response.ok) {
...
const showMessage = () => {
    ...
    if (correctAnswers < 5) {
        ...
        imageSrc = "Image/image-assez-bien.jpg";
    }  else {
        message = "Bravo ! Vous êtes prêt à passer l'examen blanc disponible sur le site.";
