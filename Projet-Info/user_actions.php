...
function validatePassword($password) {
    return strlen($password) >= 8 && preg_match('/[A-Z]/', $password) && preg_match('/[0-9]/', $password) && preg_match('/[!@#$%^&*(),.?":{}|<>]/', $password);
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'update') {
        ...
        if (!validatePassword($new_password)) {
            $_SESSION['message'] = "Erreur : Le mot de passe doit contenir au moins 8 caractères, dont une majuscule, un chiffre et un caractère spécial.";
            header("Location: manage_account.php");
            exit();
        }
